...
def ellipse_along_axis(coord_data, axis, set_number_of_layers = 10):
    """
    ...
    for layer in range(start_layer + skip_layer, end_layer - skip_layer, skip_layer):
        ...
        if axis == "x":
            ...
            area_sum += float(layer_area.area)
        print(layer_area.area)

    area = area_sum / number_layers
    print("area =", area)
    print("number layer", number_layers)
    ...
    return pfp_x_values, pfp_y_values, pfp_z_values, Xc, Yc, Zc, height, width, length
