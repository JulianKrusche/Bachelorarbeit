...
def plypolygon (coord_data):
    ...
    coord_data.pop(0)
    #print("neuer layerpoint 0",coord_data[0])
    next = []

    while len(coord_data) >= 2:
        i = 0
        cdistance = 100000
        #print("Coord length", len(coord_data))
        for line in coord_data:
            if math.dist(start,line) < cdistance:
                next = line
                cdistance = math.dist(start,line)
                next_index = i
                #print(cdistance)
                i += 1
            else:
        ...
        #print("next",next)
        polypoints.append(next)
        #next_index = polypoints.index(next)
        #print("next pre pop", coord_data[next_index])
        coord_data.pop(next_index)
        #print("next after pop", coord_data[next_index])

        #print(next_index)
        #print(polypoints(next_index))
        start = next
